...
p1 <- ggplot(BR_complete, aes(x = Date, 
                                  y = Rainfall)) +
  geom_point(aes(color = Rainfall), size = 3, alpha = 0.9) +
...
p2 <- ggplot(BR_complete, aes(x = Date, 
                                  y = Temperature_C)) +
  geom_point(aes(color = Temperature_C), size = 3, alpha = 0.9) +
  geom_smooth(method = "glm", 
              #method.args = list(family = "poisson"),
              color = "black",
             alpha = 0.35,
...
BR_Rain_Components <- 
  as.data.frame(BR_Rain_Decomp$time.series[,1:3])
BR_Temp_Components <- 
  as.data.frame(BR_Temp_Decomp$time.series[,1:3])
