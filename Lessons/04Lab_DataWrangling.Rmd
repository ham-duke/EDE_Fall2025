...
NTL.phys.data.1999 <- NTL.phys.data %>%
  filter(year4 == 1999)
...
NTL.phys.data.Tuesday.1990.1999 <- NTL.phys.data %>% 
...
NTL.phys.data.Tuesday.1990.1999.July <- NTL.phys.data %>% 
...
exercise_4 <- NTL.phys.data.Tuesday.1990.1999.July %>% 
  filter(depth == 0)

summarize(exercise_4)

mean(exercise_4$temperature_C)
...
exc_5 <- NTL.phys.data %>% 
  pivot_longer(irradianceWater:irradianceDeck, names_to = "irradiance", values_to = "values")
